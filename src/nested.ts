...
export function sumPublishedPoints(questions: Question[]): number {
    ...
    const sum = publish.reduce(
        (total: number, publish: Question) => total + publish.points,
        0
    );
    return sum;
}
...
export function sameType(questions: Question[]): boolean {
    ...
    return types.length === 0 || types.length === questions.length;
}
...
export function editOption(
    ...
    const newArr = questions.map((question: Question) => {
        if (question.id === targetId) {
            return {
                ...question,
                options: help(targetOptionIndex, newOption, question.options)
            };
        } else {
            return { ...question };
        }
...
export function duplicateQuestionInArray(
    ...
    newArr.splice(index + 1, 0, duplicate);
    return newArr;
}
